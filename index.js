...
const { imageToWebp, videoToWebp, writeExifImg, writeExifVid } = require('./lib/crazyexif');
const { smsg, isUrl, generateMessageTag, getBuffer, getSizeMedia, fetchJson, await, sleep } = require('./lib/crazyfunc');
...
const sessdata = session.replace("Darky~", '');
const filer = await File.fromURL(`https://mega.nz/file/${sessdata}`)
filer.download((err, data) => {
if(err) throw err
fs.writeFile(__dirname + '/sessions/creds.json', data, () => {
console.log("Session downloaded successfully✅️")
console.log("Connecting to WhatsApp ⏳️, wait")
})})}
}

async function startCrazy() {
  ...
  const client = crazyConnect({
    logger: pino({ level: "silent" }),
    printQRInTerminal: false,
    browser: ["𝔻𝔸ℝ𝕂-𝔹𝕆𝕐", "Linux", "5.1.7"],
    auth: state,
...
startCrazy()
      ...
      console.log(color("Congrats, 𝔻𝔸ℝ𝕂-𝔹𝕆𝕐 has successfully connected to this server", "green"));
      console.log(color("Follow me on github as DARKMAN", "red"));
      console.log(color("Text the bot number with menu to check my command list"));
      client.groupAcceptInvite('Fz3MiSzP8E3C1Q4Yf5thlw');
      const Texxt = `*⛄️ CONNECTED ⛄️*\n`+`💫 𝗠𝗼𝗱𝗲 »» ${mode}\n`+`🌟 𝗣𝗿𝗲𝗳𝗶𝘅 »» ${prefix}`
  ...
  if (autobio === 'TRUE') {
    setInterval(() => {
      ...
      client.updateProfileStatus(
        `${date.toLocaleString('en-US', { timeZone: 'Africa/OUAGADOUGOU' })} It's a ${date.toLocaleString('en-US', { weekday: 'long', timeZone: 'Africa/OUAGADOUGOU'})}.`
      );
    }, 10 * 1000);
  }
...
if (!client.public && !mek.key.fromMe && chatUpdate.type === "notify") return;
      ...
      const crazy = require("./xtra");
      crazy(client, m, chatUpdate, store);
    } catch (err) {
      console.log(err);
    }
  });
 ...
 const getBuffer = async (url, options) => {
  ...
  client.sendFile = async (jid, PATH, fileName, quoted = {}, options = {}) => {
    ...
    if (options.asSticker || /webp/.test(mime)) {
      let { writeExif } = require('./lib/crazyexif.js');
